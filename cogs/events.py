...
class EventHandler(commands.Cog):
    ...
    async def on_lyra_node_connected(
        self, node_id: str, is_nodelink: bool, reconnect: bool
    ):
        # Nodelink/Lavalink connected event
        print(f"{node_id}: Nodelink({is_nodelink}) reconnect({reconnect})")

    @commands.Cog.listener()
    async def on_lyra_node_disconnected(
        self, node_id: str, is_nodelink: bool, player_count: int
    ):
        # Nodelink/Lavalink disconnected event
        print(f"{node_id}: Nodelink({is_nodelink}) playercount({player_count})")

    @commands.Cog.listener()
    async def on_lyra_node_reconnecting(
        self, node_id: str, is_nodelink: bool, retry_in: float
    ):
    ...
    async def on_lyra_track_stuck(
        self, player: CustomPlayer, track: lava_lyra.Track, threshold: float
    ):
        # Lavalink track stucked event
        print(f"Player stucked. Wait for {threshold}s.")

    @commands.Cog.listener()
    async def on_lyra_track_exception(
        self,
        player: CustomPlayer,
        track: lava_lyra.Track,
        error: lava_lyra.TrackExceptionEvent,
    ):
        # Nodelink/Lavalink track exception event
        print(f"Track exception: {error}.")

    @commands.Cog.listener()
    async def on_lyra_lyrics_found(
        self, player: CustomPlayer, track: lava_lyra.Track, lyrics: lava_lyra.Lyrics
    ):
        # Successfully found lyrics event
        print(f"Lyrics found for {track.title}: {len(lyrics.lines)} lines")

    @commands.Cog.listener()
    async def on_lyra_lyrics_unavailable(
        self, player: CustomPlayer, track: lava_lyra.Track
    ):
