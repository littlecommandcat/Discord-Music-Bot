...
class PresenceTask(commands.Cog):
    ...
    async def cog_unload(self):
        if (
            self.update_presence.is_running()
            and not self.update_presence.is_being_cancelled()
        ):
                    ...
                    if self.is_youtube(current.uri) or self.is_twitch(current.uri):
                        ...
                        await self.bot.change_presence(
                            activity=discord.Streaming(
                                name=player.current.title[:120], url=current.uri
                            )
                        )
                    else:
